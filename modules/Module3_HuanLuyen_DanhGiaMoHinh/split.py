...
===================== MỤC LỤC FILE =====================
[QUAN TRỌNG] group_aware_split() — chia theo NHÓM (feature_signature) chứ không theo
             dòng: 848/1000 dòng trùng vector nên chia theo dòng là leakage (bẫy #3).
             Trong mỗi lớp: xếp nhóm theo sha256(signature + seed) — deterministic,
             CÙNG quy tắc với ml_analysis.py → tái lập đúng train=697/test=303.
[QUAN TRỌNG] assert overlap == 0 — nếu 1 signature lọt cả 2 phía thì DỪNG ngay.
[QUAN TRỌNG] assert_no_conflict() — 1 signature gắn 2 nhãn khác nhau → dừng training.
Phần còn lại: add_feature_signature() (sha2 trên concat_ws '|'), đọc/ghi parquet, in đếm.
=========================================================
"""
import argparse
