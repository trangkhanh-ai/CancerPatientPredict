...
namespace CancerBigData.UI
{
    ...
    /// <remarks>
    /// ===================== MỤC LỤC FILE =====================
    /// [QUAN TRỌNG] BuildQuery()   — dịch lựa chọn trên UI thành query string khớp WHITELIST backend
    /// [QUAN TRỌNG] SearchAsync()  — gọi API bất đồng bộ (không block UI) + huỷ request cũ (CancellationToken)
    /// [QUAN TRỌNG] INDICATORS     — 21 chỉ số được phép lọc, PHẢI trùng whitelist trong query_builder.py
    /// Phần còn lại là "khung sườn" giao diện:
    ///   BuildUi()      — dựng control thủ công (combobox/numeric/nút/bảng), chỉ là layout
    ///   ResetFilters() — đưa bộ lọc về mặc định
    ///   FillGrid()     — đổ JSON trả về vào DataGridView + tô màu mức độ
    ///   ShowDetail()   — double-click 1 hàng → MessageBox toàn bộ trường của bệnh nhân
    ///   ExportAsync()  — gọi /patients/export tải CSV về (SaveFileDialog)
    /// =========================================================
    /// </remarks>
    public class PatientSearchControl : UserControl
    {
        ...
        // [QUAN TRỌNG] 21 chỉ số được phép lọc — PHẢI trùng từng chữ với SCALE_1_9_COLUMNS
        // (whitelist trong src/api/query_builder.py). Gửi tên ngoài danh sách → backend trả 422.
        ...
        // [QUAN TRỌNG] Bấm Tìm liên tục → request MỚI huỷ request CŨ, tránh việc
        // kết quả cũ về muộn đè lên kết quả mới (race condition kinh điển của UI async).
        ...
        /// <summary>
        /// [QUAN TRỌNG] Trái tim của tab này: dịch trạng thái UI → PatientQuery.
        /// Quy tắc khớp hợp đồng API /patients:
        ///   - ComboBox toán tử: "=" → eq · "≥" → gte · "≤" → lte · "trong khoảng" → between
        ///   - between gửi min_value/max_value (tự hoán đổi nếu người dùng nhập ngược)
        ///   - gender: index combobox 1/2 = Nam/Nữ đúng mã hoá trong MongoDB
        ///   - chỉ gửi tham số KHÁC mặc định (tuổi 0/120 = không lọc)
        /// Sai bất kỳ giá trị nào so với whitelist → backend trả 422, không lọt vào Mongo.
        /// </summary>
        ...
        /// <summary>
        /// [QUAN TRỌNG] Gọi GET /patients hoàn toàn async (await) — UI KHÔNG bị đứng.
        /// Tuyệt đối không dùng .Result / .Wait() (sẽ deadlock UI thread).
        /// Mỗi lần gọi: huỷ token cũ → tạo token mới → nếu request này bị huỷ giữa chừng
        /// thì bỏ qua kết quả (OperationCanceledException).
        /// </summary>
