...
namespace CancerBigData.UI
{
    ...
    /// <remarks>
    /// ===================== MỤC LỤC FILE =====================
    /// [QUAN TRỌNG] LoadAsync()    — gọi GET /correlation; backend đã tính sẵn rank/impact,
    ///              client CHỈ hiển thị (mọi tính toán nằm ở correlation_service.py)
    /// [QUAN TRỌNG] DrawRanking()  — thanh ngang top-12 theo impact; độ dài tỉ lệ với
    ///              impact/max; màu theo hạng (top 3 đỏ, 4–6 vàng, còn lại xanh)
    /// Phần còn lại:
    ///   FillGrid()  — bảng chi tiết TB Low/Medium/High + Δ + % High khi chỉ số 7–9
    ///   ExportCsv() — ghi bảng ra CSV bằng StringBuilder (SaveFileDialog)
    ///   BuildUi()   — layout (khung sườn)
    /// =========================================================
    /// </remarks>
