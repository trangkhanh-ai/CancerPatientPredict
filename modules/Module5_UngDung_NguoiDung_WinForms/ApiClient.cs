...
{
    // ============================== MỤC LỤC FILE ==============================
    // Nửa trên  — các DTO: lớp C# "soi gương" đúng JSON của API (JsonPropertyName
    //             phải trùng từng chữ với key snake_case backend trả về).
    // Nửa dưới  — ApiClient: nơi DUY NHẤT gọi HTTP trong toàn bộ app.
    // [QUAN TRỌNG] _http là HttpClient STATIC dùng chung — tạo mới mỗi request sẽ
    //              cạn socket (socket exhaustion), đây là lỗi C# kinh điển.
    // [QUAN TRỌNG] LoadBaseUrl() — đọc ApiBaseUrl từ appsettings.json; base URL
    //              PHẢI có "/api/v1/" và dấu "/" cuối, thiếu là mọi request 404.
    // [QUAN TRỌNG] ApiException — mang mã HTTP để UI phân biệt 422 (nhập sai)
    //              với 503 (model chưa nạp) mà hiển thị thông báo khác nhau.
    // Phần còn lại: các method Get*/Predict* chỉ là "gọi endpoint + parse JSON".
    // ==========================================================================
