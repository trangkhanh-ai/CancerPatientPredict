...
namespace CancerBigData.UI
{
    ...
    /// <remarks>
    /// ===================== MỤC LỤC FILE =====================
    /// [QUAN TRỌNG] LoadAsync()  — gọi GET /stats async rồi Invalidate() 3 panel để vẽ lại
    /// [QUAN TRỌNG] DrawDonut()  — donut = FillPie từng lát (góc quét = 360 × n/total)
    ///              rồi phủ hình tròn trắng ở giữa làm "lỗ"
    /// Phần còn lại là vẽ/đổ dữ liệu:
    ///   DrawKpi()   — 4 thẻ KPI (tổng BN, High, Medium, Low) vẽ tay bằng FillRectangle
    ///   DrawBars()  — biểu đồ cột nhóm: trung bình 5 chỉ số × 3 mức độ, thang cố định 0–9
    ///   FillGrid()  — bảng phân bố nhóm tuổi + giới tính
    ///   BuildUi()   — layout panel/nút (khung sườn)
    /// =========================================================
    /// </remarks>
