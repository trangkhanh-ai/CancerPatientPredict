...
===================== MỤC LỤC FILE =====================
[QUAN TRỌNG] Tư tưởng chống injection: người dùng KHÔNG được tự đặt tên field hay
             toán tử — chỉ được chọn trong 3 tập cho phép:
               FILTERABLE_INDICATORS = 21 chỉ số thang 1..9
               OPERATORS             = {eq, gte, lte, between}
               SORTABLE              = {patient_id, age, level, gender} + 21 chỉ số
             Ngoài tập → raise ValueError → router trả 422.
[QUAN TRỌNG] Mọi giá trị số đều ép int(...) trước khi đưa vào query — chuỗi kiểu
             {"$where": ...} không có đường vào Mongo.
Phần còn lại: build_patient_query() ghép dict filter từng phần (level/gender/tuổi/chỉ số);
build_sort() mặc định sort theo patient_id tăng dần.
=========================================================
"""
import sys, os
