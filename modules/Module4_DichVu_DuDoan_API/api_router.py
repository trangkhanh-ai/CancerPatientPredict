...
===================== MỤC LỤC FILE (11 endpoint, prefix /api/v1) =====================
[QUAN TRỌNG] /patients/export khai báo TRƯỚC /patients/{patient_id} — FastAPI khớp
             route theo THỨ TỰ khai báo; đảo lại thì "export" bị hiểu là patient_id (bẫy #1).
[QUAN TRỌNG] predict() — bắt RuntimeError của model_service → trả 503 (model chưa nạp);
             lưu prediction vào Mongo trong try/except riêng: lưu FAIL vẫn trả kết quả
             cho người dùng (ghi sổ là phụ, dự đoán là chính).
[QUAN TRỌNG] patients() — mọi tham số lọc đi qua build_patient_query/build_sort
             (whitelist); ValueError → 422, KHÔNG để chuỗi lạ lọt vào Mongo query.
Phần còn lại: health/model đọc trạng thái model_service; predictions/stats/quality/
correlation chỉ gọi thẳng service tương ứng rồi trả JSON.
======================================================================================
"""
